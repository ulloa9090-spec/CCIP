...
### Dashboard category colors (Premium Polish, iteration 2 — ADR 0019)

Eleven `--category-*` tokens (`blue/indigo/violet/cyan/teal/orange/amber/rose/pink/sky/slate`, each with light/dark values like every other color token) exist **only** for decorative categorization on the Dashboard — `WidgetCard`'s optional `accent` prop puts the widget's icon in a small colored chip, one color per widget, via `features/dashboard/components/widget-accent.ts`'s literal class map (Tailwind can't see a dynamically interpolated class name, so this is a closed record, not a template string). `--accent` itself is untouched and remains the only color used for interactive elements app-wide — category colors never appear on a button, link, or focus ring. `ProgressRing` additionally gained a `tone` prop (`accent | success | warning | danger`) for the one place a ring's color should track a value rather than a fixed category (Weekly Score) — always paired with a visible text label, per the "never color alone" rule above.
