...
### Phase 11: isolation test for notifications and automations, no findings

Same methodology again, extended to both tables this phase added:

- **`notifications`**: User A sees 0 of User B's rows via `select`; cross-user `update` (marking read) affects 0 rows; insert with a spoofed `user_id` (User B's) rejected; own insert succeeds.
- **`automations`**: User A sees 0 of User B's rows via `select`; cross-user `delete` affects 0 rows; own insert succeeds.

Every case passed on the first run. Fixtures (two `auth.users` rows, one notification + one automation per user) and both test users deleted afterward; development database confirmed empty of test data via a follow-up count query across both fixture tables.

### Phase 11: Automation engine correctness and evaluation safety

`features/automations/match.ts`'s trigger-matching and idempotency functions are pure logic with no database dependency — `tests/automation-match.ts` (run via `npx tsx`) exercises them directly, 14 cases covering `daysBetween`, `isTaskOverdueCheckDue`, `matchingOverdueTasks` (with and without a priority condition), and `isWeeklyScheduleDue` (including the "scheduled day but before the scheduled hour" edge case). `evaluateAutomations()` (`features/automations/evaluate.ts`) runs entirely through the authenticated user's own RLS-scoped Supabase client — it never uses a service-role key, so an automation can only ever read that user's own tasks and write that user's own `notifications`/`automations` rows, regardless of what its stored `trigger_config`/`condition` jsonb contains. Called from `Header` on every page load (ADR 0014); a no-op automation (nothing due) does no query work beyond the initial `select` of enabled automations.

### Phase 11: Magic Link auth doesn't leak account existence
