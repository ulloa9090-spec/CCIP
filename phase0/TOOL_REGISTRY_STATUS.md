...
| Height | `PROTOTYPE` (iOS only) | Dedicated 2-tap tool (`MeasureToolMode.height`): tap the base point, then the top point — auto-finishes and reports the vertical (gravity/world-up constrained) height, per `docs/25_MEASUREMENT_TOOLS_CATALOG.md` section 5. ARKit's default `.gravity` world alignment already makes the world Y axis vertical, so this is the Y-difference between the two taps, not the raw 3D distance (which would also fold in any horizontal drift between them); that drift is instead surfaced as its own "off vertical" warning (in-scene and in the result card) once it exceeds ~2cm, since there's no drag-to-correct in this AR prototype — the catalog's "allow manual correction" becomes "see the warning, Undo, re-tap." Distinct from the existing polygon-to-height-point perpendicular (reachable only after closing a 3+ point shape, for volume) — that measurement is unchanged and still exists alongside this new dedicated tool. Not run on real hardware. |
