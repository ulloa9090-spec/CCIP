...
| Angle | `PROTOTYPE` (iOS only) | Two paths now: (1) interior angle at every vertex of the current polyline/polygon (`PolygonGeometry.angleDegrees`/`interiorAngles`), plus a live preview while still placing points; (2) **new** — a dedicated 3-tap Angle tool mode (`MeasureToolMode.angle` in `ARMeasureScreen.swift`), matching the catalog's A-B-C tool: tap a ray endpoint, the vertex, then the second ray endpoint, and it auto-finishes showing the angle plus both ray lengths. No new geometry — reuses the same `interiorAngles` call on an open 3-point line. Not run on real hardware. |
